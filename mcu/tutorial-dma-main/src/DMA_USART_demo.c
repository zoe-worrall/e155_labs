...
int main(void) {
    ...
    DMA1C2->CCR |= _VAL2FLD(DMA_CCR_PL,   0b10);
    DMA1C2->CCR |= _VAL2FLD(DMA_CCR_MINC, 0b1);
    DMA1C2->CCR |= _VAL2FLD(DMA_CCR_CIRC, 0b1);
    DMA1C2->CCR |= _VAL2FLD(DMA_CCR_DIR,  0b1);

    // Set DMA source and destination addresses.
    // TODO: Source: Address of the character array buffer in memory.
    DMA1C2->CMAR  |= _VAL2FLD(DMA_CMAR_MA, (uint32_t) &CHAR_ARRAY);
    DMA1C2->CPAR  |= _VAL2FLD(DMA_CPAR_PA, (uint32_t) &(USART->TDR));

    DMA1C2->CNDTR |= CHAR_ARRAY_SIZE;
    DMA1C2->CPAR  |= USART_TDR_TDR;
